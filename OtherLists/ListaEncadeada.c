...
void inserirNoInicio(Lista* lista, int valor){
    No* novo = criarNo(valor); //criar o novo nó
    novo->prox = lista->inicio; // novo nó aponta para primeiro elemento da lista
    if(lista->inicio == NULL){ //verifico se a lista está vazia
        lista->fim = novo;      //caso a lista esteja vazia, o novo nó também é o último.
    }
    lista->inicio = novo;       //Atualiza a referência para o primeiro elemento.
    lista->tamanho++;       //Incrementa a variável tamanho.
}
